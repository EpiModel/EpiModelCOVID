...
mortality_rate <- c(
  588.45, 24.8, 11.7, 14.55, 47.85, 88.2, 105.65, 127.2, 154.3, 206.5, 309.3,
  495.1, 736.85, 1051.15, 1483.45, 2294.15, 3642.95, 6139.4, 13938.3
)
...
room.sectors <- apportion_lr(
  length(room.ids), 1:n.sectors,
...
est1 <- suppressMessages(
  netest(nw, formation1.pre, target.stats1.pre, coef.diss1,
    coef.form = -Inf,
...
est2 <- suppressMessages(
  netest(nw, formation2.pre, target.stats2.pre, coef.diss2,
    coef.form = -Inf,
    set.control.ergm = control.ergm(MCMLE.maxit = 500)
  )
)

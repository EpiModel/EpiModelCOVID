...
#' @export
deaths_covid_boost <- function(dat, at) {
...
#' @rdname moduleset-boost
#' @export
arrival_covid_boost <- function(dat, at) {
  ...
  ## Update Attr
  if (nNew > 0) {
    dat <- setNewAttr_covid_boost(dat, at, nNew)
  }
...
setNewAttr_covid_boost <- function(dat, at, nNew) {
  ...
  dat <- append_attr(dat, "strain", NA, nNew)

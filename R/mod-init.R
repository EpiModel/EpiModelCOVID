...
#' @export
init_covid_boost <- function(x, param, init, control, s) {
  ...
  ## Infection Status and Time Modules
  dat <- init_status_covid_boost(dat)

  ## Get initial prevalence
  dat <- prevalence_covid_boost(dat, at = 1)
...
init_status_covid_boost <- function(dat) {
